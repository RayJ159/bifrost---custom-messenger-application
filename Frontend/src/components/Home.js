...
function Home() {
  ...
  function updateMessages() {
    axios
      .get(`${url}/realms/${curRealm}`)
      .then(function (response) {
        var messagesdata = response["data"];
        messagesdata.sort((a, b) => {
          return (
            parseInt(a["messagetime"], 10) - parseInt(b["messagetime"], 10)
          );
        });
    ...
    var answerFunction = async (arg, argb) => {
      if (argb == key) {
        ...
        for (var i = 0; i < bufferMap[key].length; i++) {
          ...
          await axios.post(`${url}/streamice`, {
            ...bufferMap[key][i],
            realm: email,
            viewer: key,
          });
  ...
  useEffect(() => {
    if (viewable) {
      ...
      socket.on(curRealm + "-streaming", () => {
        axios.post(`${url}/${curRealm}/viewer`, {
          viewer: email + "-" + curRealm,
        });
      });
      axios.post(`${url}/${curRealm}/viewer`, {
  ...
  function sendMessage(text) {
    setMessages([
      ...messages,
      { email: email, messagetext: text, messagetime: Date.now() },
    ]);
    axios
      .post(`${url}/realms/${curRealm}`, { email: email, text: text })
      .then((e) => {
        console.log(e);
      })
      .catch((e) => {
    ...
    <div className="wrapper">
        ...
        <div>
          {!viewable ? (
              ...
              <button
                onClick={async () => {
                  ...
                  const stream = await navigator.mediaDevices.getUserMedia({
                    video: { facingMode: facing, aspectRatio: {ideal: 16.0/9.0}},
                    audio: true,
                  });
            ...
            <button onClick={async () => {
                streamRef.current.srcObject.getAudioTracks()[0].enabled = false;
                setMute(true);
            }}>mute</button>
          ) : (
            <button onClick={async () => {
                streamRef.current.srcObject.getAudioTracks()[0].enabled = true;
                setMute(false)
            }}>unmute</button>
          )}
        </div>
      ...
      <div className="mainBody">
        <div className="videoDiv">
          {!viewable && (
            <video className="video" muted autoPlay ref={webcamRef}></video>
          )}
          {viewable && (
            <video className="video" autoPlay ref={streamRef}></video>
          )}
          {!viewable && (
            <button
              onClick={() => {
